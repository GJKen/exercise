	...
	class Lightbox {
      ...
      `;
		...
		open() {
			...
			// 获取 lb-lightbox-overlay 元素
			let lightboxOverlay = document.querySelector('.lb-lightbox-overlay');

			// 给 lb-lightbox-overlay 添加点击事件监听器
			lightboxOverlay.addEventListener('click', function(event) {
				// 判断点击的目标是否为 lightboxOverlay 元素本身（防止点击内部元素时触发）
				if (event.target === lightboxOverlay) {
					// 这里是点击 lb-lightbox-overlay 时执行的代码
					console.log("点击了 lb-lightbox-overlay，执行其它操作");
